...
*  可以有不同类型的操作数的表达式称为**混合模式表达式**，支持这种表达式的语言必须定义用于操作数隐式转换类型的协议，因为计算机中没有使用不同类型操作数的二元运算符。
*  **强制转换**定义为一种由**编译器**执行的**隐式转换**。**程序员**显式要求的类型转换称为**显式转换**，而非强制转换。

#### 7.4.2 显式类型转换

*  在基于C的语言中，显式类型转换叫做强制类型转换。
	* （int）angle

#### 7.4.3 表达式中的错误

*   类型错误
*   浮点溢出，操作的结果无法在内存单元中存储
	* 上溢，结果过大
	* 下溢，结果过小
	* 除以0
...
*  如果表达式的结果不是在计算了所有的操作数和运算符之后得出的，就叫做表达式的短路求值。
	* 例如， (13*a) * (b/13 -1)
		* 如果 a = 0,就没有必要计算(b/13 -1)
		* 但在程序执行过程中，算术表达式中的短路求值是不容易检测到的。因此这样的短路求值从来不会发生
	* 布尔表达式
		* (a>=0)&&(b<10)
		* 如果a<0 ,则布尔表达式的值与第二个关系表达式无关。
		* 这种短路求值在执行过程中很容易检测到  
* 若语言支持布尔表达式的短路求值，但对表达式有副作用，就会发生难以捉摸的错误。 
...
*  目前流行的几乎所有编程语言都使用等号作为赋值运算符。
* 所有这些语言都必须使用与等号不同的符号来表示相等的关系运算符，以避免与赋值运算符相混淆。
